...
public class ResellerReport {
	...
	// select CONCAT(MedicalMineResellers.RESELLER_ID, ',', REPLACE(RESELLER_NAME,
	// ',', '.'), ',', PRACTICE_ID, ',', REPLACE(PRACTICE_NAME, ',', '.'), ',',
	// FROM_UNIXTIME(TIME_OF_CREATION/1000)) from db1odb.PracticesList INNER JOIN
	// db1odb.MedicalMineResellers ON
	// MedicalMineResellers.RESELLER_ID=PracticesList.RESELLER_ID
	public static void main(String[] args) throws Exception {
		// TODO Auto-generated method stub
		String fileName = CommonUtil.resellerDir.concat("/resellers.csv");
			...
			if (!customerId.isEmpty()) {
				...
				while (i < size) {
					...
					if ((invoice_number.matches(subInvFormat)
							&& ((d1 >= subStartDate.getTime()) && (d1 < subEndDate.getTime())))
							|| (!invoice_number.matches(subInvFormat)
									&& ((d1 >= othersStartDate.getTime()) && (d1 < othersEndDate.getTime())))) {
						//
						if (invoice_number.matches(subInvFormat)) {
